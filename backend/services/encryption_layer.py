...
from backend.security.key_derivation import get_cipher, hash_sha256
...
class EncryptionLayer:
    ...
    def cache_key(self, normalized_query: str) -> str:
        return hash_sha256(normalized_query)
    ...
    def set_cached_response(
        self, normalized_query: str, response_text: str, mode: str
    ) -> None:
        key = self.cache_key(normalized_query)
        path = os.path.join(self._cache_dir, f"{key}.enc")
        entry = {
    ...
    def log_query(
        ...
        entry = {
            ...
            "sanitized_query_hash": hash_sha256(sanitized_query),
